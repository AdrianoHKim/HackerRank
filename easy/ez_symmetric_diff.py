...
Both the discard() and remove() functions take a single value as
an argument and removes that value from the set.
If that value is not present, discard() does nothing, but remove()
...
def symmetric_difference(M, set_M, N, set_N):
    # Convert space-separated integers to sets
